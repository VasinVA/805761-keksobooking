...
var renderAd = function (ad) {
  ...
  popupPhotoTemplate.setAttribute('src', '' + PHOTOS[0]);
...
// ставим филдсетам дисэйблд, т.к. поля форм должны быть неактивны в исходном состоянии
var fieldsetFormHeader = adForm.querySelector('.ad-form-header');
fieldsetFormHeader.setAttribute('disabled', '');

var fieldsetFormElements = adForm.querySelectorAll('.ad-form__element');
fieldsetFormElements.forEach(function (element) {
  element.setAttribute('disabled', '');
});

// переводим страницу в активный режим по клику (потом надо будет переделать на перетаскивание)
...
var makeActive = function () {
  ...
  // делаем филдсеты активными
  fieldsetFormHeader.removeAttribute('disabled', '');
  fieldsetFormElements.forEach(function (element) {
...
var getCoordinates = function (element) {
  var xCoordMainPin = element.style.left;
  var yCoordMainPin = element.style.top;
  var coordinates = xCoordMainPin.substring(0, xCoordMainPin.length - 2) + ', ' + yCoordMainPin.substring(0, yCoordMainPin.length - 2);
  return coordinates;
};
